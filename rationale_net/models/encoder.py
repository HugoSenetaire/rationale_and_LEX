...
class EncoderMultipleImputation(nn.Module):

    def __init__(self, embeddings, args, imputer):
        super(EncoderMultipleImputation, self).__init__()
        ### Encoder
        self.args = args
        vocab_size, hidden_dim = embeddings.shape
        self.embedding_dim = hidden_dim
    ...
    def forward(self, x_indx, mask=None):
        '''
            ...
            mask: Mask to apply over embeddings for tao ratioanles
        '''
        print("ENTER X", x_indx.shape)
        if mask is not None :
            aux_x = self.imputer(x_indx, mask, 1).reshape(mask.shape)
            x_indx = x_indx.reshape(mask.shape)
            imputed_x = x_indx * mask + aux_x * (1 - mask)
        else :
            imputed_x = x_indx.reshape(mask.shape)
        x = self.embedding_layer(imputed_x.to(torch.long))
        if self.args.cuda:
            x = x.cuda()
